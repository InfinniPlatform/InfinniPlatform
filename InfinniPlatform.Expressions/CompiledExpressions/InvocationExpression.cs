...
{
	sealed class InvocationExpression : ICompiledExpression
	{
		...
		public InvocationExpression(string methodName, Type[] genericArguments, ICompiledExpression invokeTarget, IEnumerable<ICompiledExpression> invokeArguments)
		{
		...
		public object Execute(object dataContext, ExpressionScope scope)
		{
			...
			{
				invokeArguments = _invokeArguments.Select(i => (i != null) ? i.Execute(dataContext, scope) : null).ToArray();
			}
