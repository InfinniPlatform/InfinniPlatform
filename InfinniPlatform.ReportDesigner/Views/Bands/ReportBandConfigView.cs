...
namespace InfinniPlatform.ReportDesigner.Views.Bands
{
    ...
    sealed partial class ReportBandConfigView : Form
    {
        ...
        private void SetAllowedActions()
        {
            ...
            AddGroupFooterBtn.Enabled = (hasReportPage && selectedBand is GroupHeaderBand &&
                                         ((GroupHeaderBand) selectedBand).GroupFooter == null);

            // Можно добавлять блок данных, если это дочерний блок или у отчета нет блока данных
            AddDataBandBtn.Enabled = (hasReportPage &&
                                      (selectedBand is DataBand ||
                                       ReportPage.AllObjects.OfType<DataBand>().Any() == false));

            // Можно удалять любой блок, если это не блок данных, либо у блока данных нет групп
            DeleteBtn.Enabled = (hasReportPage && selectedBand != null &&
                                 (selectedBand is DataBand == false || selectedBand.Parent as GroupHeaderBand == null));
