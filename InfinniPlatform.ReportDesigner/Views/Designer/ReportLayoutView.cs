...
namespace InfinniPlatform.ReportDesigner.Views.Designer
{
	...
	sealed partial class ReportLayoutView : UserControl
	{
		...
		private static void DeleteDesignerConfiguration()
		{
			try
			{
				File.Delete(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "FastReport", "FastReport.config"));
			}
			catch
			{
			}
		...
		private static void PrepareControl(Func<Control, bool>[] handlers, ICollection<Control> controls, Control control)
		{
		...
		{
			var newDataSource = new BusinessObjectDataSource { Name = newDataSourceInfo.Name, ReferenceName = newDataSourceInfo.Name };
		...
		private static void CreateDataSourceProperties(Column parent, DataSchema schema)
		{
			foreach (var property in schema.Properties)
			{
				...
				{
					propertyColumn = new Column { Name = propertyName, ReferenceName = propertyName, DataType = DataTypes[propertySchema.Type] };
				}
			...
			var newTotal = new Total
							   {
								   ...
								   Evaluator = Report.AllObjects.Cast<object>().OfType<DataBand>().FirstOrDefault(i => i.Name == newTotalInfo.DataBand),
								   PrintOn = Report.AllObjects.Cast<object>().OfType<BandBase>().FirstOrDefault(i => i.Name == newTotalInfo.PrintBand),
		...
		public IEnumerable<DesignerPrintBand> GetPrintBands()
		{
			...
			if (page != null)
			{
				result.AddRange(page.AllObjects.Cast<object>().OfType<GroupFooterBand>().Select(i => new DesignerPrintBand(i)));
		...
		{
			var page = GetReportPage();
			page.ReportTitle = (page.ReportTitle == null) ? new ReportTitleBand { Name = "ReportTitle1", Height = Units.Centimeters } : null;
			RefreshReport();
		}
		...
		{
			var page = GetReportPage();
			page.ReportSummary = (page.ReportSummary == null) ? new ReportSummaryBand { Name = "ReportSummary1", Height = Units.Centimeters } : null;
			RefreshReport();
		}
		...
		{
			var page = GetReportPage();
			page.PageHeader = (page.PageHeader == null) ? new PageHeaderBand { Name = "PageHeader1", Height = Units.Centimeters } : null;
			RefreshReport();
		}
		...
		{
			var page = GetReportPage();
			page.PageFooter = (page.PageFooter == null) ? new PageFooterBand { Name = "PageFooter1", Height = Units.Centimeters } : null;
			RefreshReport();
		}
