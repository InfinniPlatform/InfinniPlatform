...
namespace InfinniPlatform.ReportDesigner.Views.DataSources
{
	...
	sealed partial class SqlDataSourceView : UserControl
	{
		...
		/// </summary>
		public DialogView<SqlTableSelectView> DataSchemaSelectDialog
		{
			get;
			set;
		}
			...
			set
			{
				if (value != null)
				{
					...
					if (provider != null)
					{
						...
						CommandTimeoutEdit.Value = (provider.CommandTimeout > 0) ? (decimal)provider.CommandTimeout / 1000 : 0;
		...
		private void OnImportDataSchema(object sender, EventArgs e)
		{
			...
			{
				DataSchemaSelectDialog.View.AsyncLoadDataSourceInfo(this, dataSourceInfo => DataSourceInfo = dataSourceInfo);
			}
		}
