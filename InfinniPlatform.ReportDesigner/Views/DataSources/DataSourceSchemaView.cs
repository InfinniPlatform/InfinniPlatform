...
namespace InfinniPlatform.ReportDesigner.Views.DataSources
{
	...
	sealed partial class DataSourceSchemaView : UserControl
	{
		...
		public DataSchema DataSourceSchema
		{
			get
			{
				return BuildDataSourceSchema();
			}
			set
			{
				RenderDataSourceSchema(value);
			}
		}
		...
		public SchemaDataType SelectedPropertyType
		{
			get
			{
				return GetNodeDataType(DataSourceTree.SelectedNode);
			}
		}
		...
		public string SelectedPropertyPath
		{
			get
			{
				return GetDataSourcePropertyPath();
			}
			set
			{
				SetDataSourcePropertyPath(value);
			}
		}
		...
		public event EventHandler OnImportDataSchema
		{
			...
			remove
			{
				_onImportDataSchema = (EventHandler)Delegate.Remove(_onImportDataSchema, value);
			}
		}
		...
		private void EditProperty()
		{
			...
			if (_propertyDialog.ShowDialog(this) == DialogResult.OK)
			{
				...
				if (propertyType != SchemaDataType.Object && propertyType != SchemaDataType.Array && propertyNode.Nodes.Count > 0)
				{
		...
		private static bool IsContainerNode(TreeNode node)
		{
			...
			return (nodeDataType == SchemaDataType.None || nodeDataType == SchemaDataType.Object || nodeDataType == SchemaDataType.Array);
		}
