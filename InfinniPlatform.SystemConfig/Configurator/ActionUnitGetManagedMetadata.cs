	...
	public class ActionUnitGetManagedMetadata
	{
		public void Action(IApplyContext target)
		{
			var validator = ValidationBuilder.ForObject(builder => builder.And(rules => rules
				.IsNotNullOrEmpty("Configuration", Resources.ConfigurationIdNotFound)
				.IsNotNullOrEmpty("MetadataObject", Resources.ErrorMetadataNotSpecified)));
			...
			if (validator.Validate(target.Item))
			{
				...
				else
				{
					...
					if (generatorMetadataItem == null)
					{
						...
						target.ValidationMessage = string.Format(Resources.GeneratorMetadataNotRegistered, target.Item.MetadataType);
						return;
					}

					if (generatorMetadataItem.MetadataType != null && generatorMetadataItem.MetadataType.ToLowerInvariant() == target.Item.MetadataType.ToLowerInvariant())
					{
						var body = new
						{
