...
namespace InfinniPlatform.SystemConfig.Initializers
{
	public sealed class JsonConfigurationsInitializer : IStartupInitializer
	{
		...
		private void InstallConfiguration(string version, string configurationId)
		{
			...
			var installer = CreateJsonConfigurationInstaller(version);
		...
		private JsonConfigurationInstaller CreateJsonConfigurationInstaller(string version)
		{
			...
			IEnumerable<dynamic> configs = _indexFactory.BuildVersionProvider("systemconfig", "metadata",RoutingExtensions.SystemRouting, version).GetDocument(null,0,100000);
			Dictionary<string, string> configDictionary = configs.ToDictionary(c => (string)c.Id, c => (string)c.Name);

			IEnumerable<dynamic> documents = _indexFactory.BuildVersionProvider("systemconfig", "documentmetadata", RoutingExtensions.SystemRouting, version).GetDocument(null, 0, 1000000);
			Dictionary<string, dynamic> documentDictionary = documents.ToDictionary(c => (string) c.Id, c => c);

			IEnumerable<dynamic> registers = _indexFactory.BuildVersionProvider("systemconfig", "registermetadata", RoutingExtensions.SystemRouting, version).GetDocument(null, 0, 1000000);
			foreach (dynamic register in registers)
			{
				register.ConfigId = register.ParentId;
			}

			IEnumerable<dynamic> menu = _indexFactory.BuildVersionProvider("systemconfig", "menumetadata", RoutingExtensions.SystemRouting, version).GetDocument(null, 0, 1000000);
		...
		{
			var items = _indexFactory.BuildVersionProvider("systemconfig", metadataType, RoutingExtensions.SystemRouting, null).GetDocument(null,0,1000000);
			foreach (dynamic item in items)
			{
