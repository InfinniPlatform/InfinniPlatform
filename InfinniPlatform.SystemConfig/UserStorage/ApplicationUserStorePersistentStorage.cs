...
namespace InfinniPlatform.SystemConfig.UserStorage
{
    public sealed class ApplicationUserStorePersistentStorage : IApplicationUserStore
    {
        ...
        public void AddUserToRole(ApplicationUser user, string roleName)
        {
            ...
            dynamic role =
                new DocumentApiUnsecured(null).GetDocument(AuthorizationStorageExtensions.AuthorizationConfigId,
                                                           ...
                                                           f =>
                                                           f.AddCriteria(cr => cr.Property("Name").IsEquals(roleName)),
                                                           0, 1).FirstOrDefault();
            ...
            List<ForeignKey> roles = user.Roles.ToList();
            ...
            if (role != null && roles.FirstOrDefault(r => r.Id == role.Id) != null)
            {
                ...
                dynamic userRole =
                    new DocumentApiUnsecured(null).GetDocument(AuthorizationStorageExtensions.AuthorizationConfigId,
                                                               ...
                                                               f =>
                                                               f.AddCriteria(
                                                                   cr => cr.Property("RoleName").IsEquals(roleName))
                                                                .AddCriteria(
                                                                    cr =>
                                                                    cr.Property("UserName").IsEquals(user.UserName)), 0,
                                                               1).FirstOrDefault();
        ...
        {
            List<ApplicationUserLogin> logins =
                user.Logins.ToList()
                    .Where(f => !(f.Provider == userLogin.Provider && f.ProviderKey == userLogin.ProviderKey))
                    .ToList();
            user.Logins = logins;
            UpdateUser(user);
        }
        ...
        public void RemoveRole(string roleName)
        {
            ...
            dynamic roleLinks =
                new DocumentApiUnsecured(null).GetDocument(AuthorizationStorageExtensions.AuthorizationConfigId,
                                                           ...
                                                           f =>
                                                           f.AddCriteria(
                                                               cr => cr.Property("Roles.DisplayName").IsEquals(roleName)),
                                                           0, 1).FirstOrDefault();
            ...
            if (roleLinks != null)
            {
                throw new ArgumentException(
                    string.Format("Role with name \"{0}\" has user linked and should not be deleted", roleName));
            }

            new DocumentApiUnsecured(null).DeleteDocument(AuthorizationStorageExtensions.AuthorizationConfigId,
                                                          AuthorizationStorageExtensions.RoleStore, role.Id);
        }
        ...
        public dynamic FindClaimType(string claimType)
        {
            dynamic claim =
                new DocumentApiUnsecured(null).GetDocument(AuthorizationStorageExtensions.AuthorizationConfigId,
                                                           ...
                                                           f =>
                                                           f.AddCriteria(cr => cr.Property("Name").IsEquals(claimType)),
                                                           0, 1)
                                              .FirstOrDefault();
            if (claim != null)
            {
            ...
            if (claim != null)
            {
                dynamic claimLinks =
                    new DocumentApiUnsecured(null).GetDocument(AuthorizationStorageExtensions.AuthorizationConfigId,
                                                               ...
                                                               f.AddCriteria(
                                                                   cr =>
                                                                   cr.Property("Claims.Type.DisplayName")
                                                                     .IsEquals(claimType)), 0, 1)
                                                  .FirstOrDefault();

                if (claimLinks != null)
                {
                    throw new ArgumentException(
                        string.Format("Can't delete user claim \"{0}\": existing users have link to claim type.",
                                      claimType));
                }
                new DocumentApiUnsecured(null).DeleteDocument(AuthorizationStorageExtensions.AuthorizationConfigId,
        ...
        {
            new DocumentApiUnsecured(null).DeleteDocument(AuthorizationStorageExtensions.AuthorizationConfigId,
                                                          AuthorizationStorageExtensions.AclStore, aclId);
        }
    }
    ...
    public static class ApplicationUserStorePersistentStorageExtensions
    {
        public static void CheckStorage()
        {
            ...
            List<dynamic> acl = aclApi.GetAcl(false).ToList();

            dynamic rolesAnonimous = acl.FirstOrDefault(a => a.UserName == AuthorizationStorageExtensions.AnonimousUser);
            if (rolesAnonimous == null)
            {
                adminApi.AddAnonimousUserAcl(AuthorizationStorageExtensions.AnonimousUser);
            }

            IEnumerable<dynamic> roles = aclApi.GetRoles(false);
            //если отсутствует роль админа, добавляем ее
            dynamic roleAdmin = roles.FirstOrDefault(a => a.Name == AuthorizationStorageExtensions.AdminRole);

            if (roleAdmin == null)
            {
                aclApi.AddRole(AuthorizationStorageExtensions.AdminRole, AuthorizationStorageExtensions.AdminRole,
                               AuthorizationStorageExtensions.AdminRole);
            }

            IEnumerable<dynamic> users = aclApi.GetUsers(false);
