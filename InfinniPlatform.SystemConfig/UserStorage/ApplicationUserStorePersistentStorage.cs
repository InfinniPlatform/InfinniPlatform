...
namespace InfinniPlatform.SystemConfig.UserStorage
{
	...
	public static class ApplicationUserStorePersistentStorageExtensions
	{
		public static void CheckStorage()
		{
			...
			if (roleAdmin == null)
			{
				authApi.AddRole(AuthorizationStorageExtensions.AdminRole, AuthorizationStorageExtensions.AdminRole, AuthorizationStorageExtensions.AdminRole);
			}

			var users = authApi.GetUsers();
			if (users.FirstOrDefault(u => u.UserName == AuthorizationStorageExtensions.AdminUser) == null)
			{
				authApi.AddUser(AuthorizationStorageExtensions.AdminUser, AppSettings.GetValue("AdminPassword", "Admin"));

				//проверяем, удалось ли добавить пользователя
				users = authApi.GetUsers();
				if (users.FirstOrDefault(u => u.UserName == AuthorizationStorageExtensions.AdminUser) == null)
				{
				...
				authApi.AddUserToRole(AuthorizationStorageExtensions.AdminUser, AuthorizationStorageExtensions.AdminRole);
