...
namespace InfinniPlatform.SystemConfig.RoutingFactory
{
    public sealed class RoutingFactoryBase : IIndexRoutingFactory
    {
        ...
        {
            if (indexName.ToLowerInvariant() == "systemconfig" || indexName.ToLowerInvariant() == "authorization" ||
                indexName.ToLowerInvariant() == "restfulapi" || indexName.ToLowerInvariant() == "update")
            {
