...
namespace InfinniPlatform.Core.Abstractions.Dynamic
{
    public static class ObjectHelper
    {
        ...
        public static void SetProperty(this object target, string propertyPath, object propertyValue)
        {
            if (target == null)
            {
                throw new ArgumentNullException(nameof(target), Resources.TargetObjectCannotBeNull);
            }

            if (string.IsNullOrEmpty(propertyPath))
            {
                throw new ArgumentNullException(nameof(propertyPath), Resources.PropertyPathCannotBeNullOrEmpty);
            }
