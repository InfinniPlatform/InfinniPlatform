	...
	public static class SampleMetadataBuilder
	{
		...
		public static object BuildSampleMenu(this object instance, string menuName, string menuId)
		{
			...
			var metadata = metadataObject
				...
				.BuildMenuItem("Основное", mi => mi
					.BuildMenuItem("Вложенный menuitem", min => min.BuildOpenViewAction(ov => ov.BuildSelectListView(
						slv => slv
		...
		public static object BuildSampleView(string viewName, string viewId)
		{
			...
			var metadata = new DynamicWrapper()
				...
				.BuildName(viewName)
				.BuildDataSourcesList(dsl => dsl.BuildDataSource(ds => ds.BuildClassifierDataSource("ListDataSource", "ClassifierStorage", (string)classifierMetadata.Id)))
				.BuildStackLayoutPanel("StackPanel1", sl => sl
					...
					.BuildDataElement(de => de.BuildToolBar("ToolBar1", dt => dt
						.BuildButton(bt => bt
							.BuildButtonProperties("AddButton", "Добавить", bp => bp.BuildButtonActionAddDefault("ListDataSource", ds => ds.BuildClassifierDataSource("EditDataSource", "ClassifierStorage", (string)classifierMetadata.Id)))
							)
						.BuildButton(bt => bt
							.BuildButtonProperties("ChangeButton", "Изменить", bp => bp.BuildButtonActionEditDefault("ListDataSource", "ListDataSource", ds => ds.BuildClassifierDataSource("EditDataSource", "ClassifierStorage", (string)classifierMetadata.Id)))
							)
