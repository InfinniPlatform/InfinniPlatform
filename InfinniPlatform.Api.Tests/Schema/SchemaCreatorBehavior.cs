...
namespace InfinniPlatform.Api.Tests.Schema
{
	...
	public sealed class SchemaCreatorBehavior
	{
		...
		public void ShouldCreateSchema()
		{
			...
			var item1 = new SchemaObject(item, "Address", "Address", linkAddress, schemaAddress); //select: Patient.Address
			 
			var item2 = new SchemaObject(item1, "Street", "Street", linkStreet, null);  //select: //Patient.Address.Street

			var item3 = new SchemaObject(item, "Policies.$", "Policies",linkPolicies,schemaPolicy);

			var item4 = new SchemaObject(item1, "DisplayName", "DisplayName", linkAddressDisplayName, null);  //Patient.Address.DisplayName


			var schemaCreator = new SchemaCreator();
			var schema = schemaCreator.BuildSchema(new[] {item1, item2,item3, item4});
