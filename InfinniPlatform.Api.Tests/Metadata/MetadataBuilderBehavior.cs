	...
	public sealed class MetadataBuilderBehavior
	{
		...
		public void ShouldBuildMetadataListView()
		{
			...
			var metadata = metadataObject
				...
				.BuildDataSourcesList(dsl => dsl.BuildDataSource(ds => ds.BuildClassifierDataSource("ListDataSource", configId, (string)classifierMetadata.Id)))
				.BuildStackLayoutPanel("StackPanel1", sl => sl
					.BuildDataElement(de => de.BuildSearchPanel("SearchPanel1", sp => sp.BuildDataBinding(db => db.BuildPropertyBinding("ListDataSource"))))
					.BuildDataElement(de => de.BuildDataGrid("DataGrid1", dg => dg
						...
						.BuildDataBinding(db => db.BuildPropertyBinding("ListDataSource"))
						))
					.BuildDataElement(de => de.BuildToolBar("ToolBar1", dt => dt
						.BuildButton(bt => bt
							.BuildButtonProperties("AddButton", "Добавить", bp => bp.BuildButtonActionAddDefault("ListDataSource", ds => ds.BuildClassifierDataSource("EditDataSource", configId, (string)classifierMetadata.Id)))
							)
						.BuildButton(bt => bt
							.BuildButtonProperties("ChangeButton", "Изменить", bp => bp.BuildButtonActionEditDefault("ListDataSource", "ListDataSource", ds => ds.BuildClassifierDataSource("EditDataSource", configId, (string)classifierMetadata.Id)))
							)
						.BuildButtonSeparator()
						.BuildButton(bt => bt
							.BuildButtonProperties("DeleteButton", "Удалить", bp => bp.BuildButtonActionDelete("ListDataSource")))
						)));
		...
		public void ShouldBuildMetadataEditView()
		{
			...
			var metadata = metadataObject
				...
				.BuildDataSourcesList(dsl => dsl.BuildDataSource(ds => ds.BuildClassifierDataSource("EditDataSource", configId, (string)classifierMetadata.Id)))
				.BuildStackLayoutPanel("StackPanel1", slp => slp.BuildDataElement(de => de.BuildPropertyGrid("PropertyGrid1",
					pg => pg.BuildPropertyGridCategory("Основные реквизиты справочника", pgc => pgc
						.BuildPropertyGridTextBox("Идентификатор записи", "ID", ed => ed .BuildEditValueBinding(evb => evb.BuildPropertyBinding("EditDataSource", "ID")))
						.BuildPropertyGridTextBox("Код элемента", "Code",ed => ed.BuildEditValueBinding(evb => evb.BuildPropertyBinding("EditDataSource", "Code")))
						.BuildPropertyGridTextBox("Наименование элемента","Name", ed => ed.BuildEditValueBinding(evb => evb.BuildPropertyBinding("EditDataSource", "Name")))
						))))
				.BuildToolBar("Toolbar1", tb => tb
					.BuildButton(bt => bt.BuildButtonProperties("SaveButton", "Сохранить", ab => ab.BuildButtonActionSave("EditDataSource")))
					.BuildButton(bt => bt.BuildButtonProperties("CancelButton", "Отменить", cb => cb.BuildButtonActionCancel("EditDataSource")))
					);
		...
		public void ShouldBuildHomePage()
		{
			...
			var metadata = metadataObject
				...
				.BuildTabPanel("Рanel1", pn => pn.BuildTabPage("Конфигурация: справочники НСИ", pa => pa
					.BuildDisableClose()
					.BuildStackLayoutPanel("StackPanel1", slp => slp
						.BuildActionElement(ae => ae.BuildMenuBar("Menu1","ClassifierStorage")))
					));
		...
		public void ShouldBuildMetadataMenu()
		{
			...
			var metadata = metadataObject
				...
				.BuildMenuItem("Основное", mi => mi
					.BuildMenuItem("Просмотр справочников НСИ", min => min.BuildOpenViewAction(ov => ov.BuildSelectListView(
						slv => slv
								.BuildText("Выберите классификатор")
								.BuildDataSourcesList(dsl => dsl.BuildDataSource(ds => ds.BuildMetadataDataSource("SelectDataSource","ClassifierStorage", "Classifier")))
								.BuildStackLayoutPanel("StackPanel1", sl => sl
