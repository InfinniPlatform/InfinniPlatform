...
namespace InfinniPlatform.Api.Tests.Serialization
{
	...
	public sealed class JsonObjectSerializerTest
	{
		...
		public void ShouldSerializeUnknownType()
		{
			...
			// When
			var result = SerializeAndDeserialize(target);
		...
		public void ShouldSerializeKnownType()
		{
			...
			// When
			var result = SerializeAndDeserialize(target, knownTypes);
		...
		{
			// Given
			var knownTypes = new KnownTypesContainer().Add<Employee>("employee").Add<Bum>("bum");
		...
		{
			// Given
			var knownTypes = new KnownTypesContainer().Add<ProductCategory>("Abstraction1").Add<Milk>("milk").Add<Bread>("bread");
			var milk = new Milk { Caption = "Первый вкус", Protein = 2.9f };
			var bread = new Bread { Caption = "Бородинский", Richness = 365 };
			var target = new ProductCategory { Products = new IProduct[] { milk, bread } };
		...
		{
			var serializer = new JsonObjectSerializer(knownTypes);
	...
	class OrderHistory : IEnumerable<Order>
	{
