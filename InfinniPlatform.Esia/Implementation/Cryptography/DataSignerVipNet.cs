...
{
	sealed class DataSignerVipNet : IDataSigner
	{
		public DataSignerVipNet(X509Certificate2 certificate)
		{
			if (certificate == null)
			{
		...
		public byte[] CreateSignature(string data)
		{
			...
			// Инфорация о подписи
			var privateKey = _certificate.GetPrivateKeyAlgorithm();
		...
		{
			var signatureDescription = GostCryptoConfig.CreateFromName(signatureKey.SignatureAlgorithm) as SignatureDescription;
