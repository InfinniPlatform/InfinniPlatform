...
namespace InfinniPlatform.Authentication.Modules
{
    internal sealed class IdentityApplicationUserManager : IApplicationUserManager
    {
        ...
        {
            ThrowIfError(_userManager.Value.CreateAsync(new IdentityApplicationUser { UserName = userName }, password));
        }
        ...
        {
            var userManager = _userManager.Value;
            var currentUserId = GetCurrentUserId();
            var task = actionOnCurrentUser(userManager, currentUserId);
            return task.Result;
        }
        ...
        {
            var userManager = _userManager.Value;
            var task = ProcessSpecifiedUser(actionOnSpecifiedUser, userManager, userName);
            return task.Result;
        }
