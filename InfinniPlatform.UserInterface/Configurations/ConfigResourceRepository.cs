...
{
	static class ConfigResourceRepository
	{
		...
		{
			var resource = string.Format(@"{0}.Configurations.{1}.Menu.{2}.resjson", Assembly.GetName().Name, configId, menuId);
		...
		{
			var resource = string.Format(@"{0}.Configurations.{1}.Documents.{2}.", Assembly.GetName().Name, configId, documentId);
			var items = Resources.Where(r => r.StartsWith(resource)).Select(ReadObject).ToArray();
			return (items.Length > 0) ? items : null;
		}
