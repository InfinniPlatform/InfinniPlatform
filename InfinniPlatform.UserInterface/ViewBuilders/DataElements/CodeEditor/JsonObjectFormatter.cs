...
namespace InfinniPlatform.UserInterface.ViewBuilders.DataElements.CodeEditor
{
	...
	public sealed class JsonObjectFormatter : IObjectFormatter
	{
		...
		private static TResult InvokeSilent<TResult>(Func<TResult> action, List<CodeEditorError> errors) where TResult : class
		{
			TResult result = null;
			...
			catch (JsonReaderException error)
			{
				...
				errors.Add(new CodeEditorError(CodeEditorErrorCategory.Error, error.Message, error.Path, error.LineNumber, error.LinePosition));
			}
			...
			catch (MetadataSchemaException error)
			{
				...
				errors.AddRange(error.Errors.Select(e => new CodeEditorError(CodeEditorErrorCategory.Warning, e.Message, e.Path, e.LineNumber, e.LineColumn)));
			}
