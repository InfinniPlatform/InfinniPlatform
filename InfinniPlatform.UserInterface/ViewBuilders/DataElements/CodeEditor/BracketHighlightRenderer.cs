...
namespace InfinniPlatform.UserInterface.ViewBuilders.DataElements.CodeEditor
{
	...
	/// </summary>
	sealed class BracketHighlightRenderer : IBackgroundRenderer
	{
		...
		public void Draw(TextView textView, DrawingContext drawingContext)
		{
			...
			if (caretOffset >= 0 && caretOffset <= text.Length)
			{
				...
				{
					char openingBracket = text[caretOffset];
					char closingBracket;

					if (OpeningBrackets.TryGetValue(openingBracket, out closingBracket))
					{
						openingBracketOffset = caretOffset;
						closingBracketOffset = FindClosingBracketOffset(text, openingBracketOffset, openingBracket, closingBracket);
					}
				}

				if (!(openingBracketOffset >= 0 && closingBracketOffset >= 0) && caretOffset > 0)
				{
					char closingBracket = text[caretOffset - 1];
					char openingBracket;

					if (ClosingBrackets.TryGetValue(closingBracket, out openingBracket))
					{
						closingBracketOffset = caretOffset - 1;
						openingBracketOffset = FindOpeningBracketOffset(text, closingBracketOffset, openingBracket, closingBracket);
					}
				}
		...
		private static int FindClosingBracketOffset(string text, int openingBracketOffset, char openingBracket, char closingBracket)
		{
			var counter = 0;
		...
		private static int FindOpeningBracketOffset(string text, int closingBracketOffset, char openingBracket, char closingBracket)
		{
			var counter = 0;
			...
			editor.Loaded += (loadedSender, loadedArgs) =>
							 {
								 ...
								 editor.TextArea.Caret.PositionChanged += (sender, e) => editor.TextArea.TextView.InvalidateLayer(KnownLayer.Background);
							 };
		}
	}
}