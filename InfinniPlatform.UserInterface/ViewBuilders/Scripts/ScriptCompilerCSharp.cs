...
namespace InfinniPlatform.UserInterface.ViewBuilders.Scripts
{
    ...
    /// </summary>
    internal sealed class ScriptCompilerCSharp : IScriptCompiler
    {
        /// <summary>
        ...
        private static Assembly BuildAssembly(IEnumerable scripts)
        {
            ...
            if (results.Errors.HasErrors)
            {
                ...
                {
                    errors.AppendFormat("Line: {0}, Column: {1}, Error: {2}", error.Line, error.Column, error.ErrorText)
                        .AppendLine();
                }
        ...
        private static string BuildCode(IEnumerable scripts)
        {
            ...
            foreach (dynamic script in scripts)
            {
                builder.AppendFormat("public static void {0}(dynamic context, dynamic arguments) {{ {1} }}", script.Name,
                    script.Body).AppendLine();
            }
