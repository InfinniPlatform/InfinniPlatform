...
namespace InfinniPlatform.RestfulApi.Extensions
{
    public static class IndexedStorageExtension
    {
		...
		{
			var provider = new ElasticFactory(new RoutingFactoryBase()).BuildIndexStateProvider();
        ...
        {
			var elasticProvider = new ElasticFactory(new RoutingFactoryBase()).BuildVersionProvider(index, typeName, AuthorizationStorageExtensions.AnonimousUser);
        ...
        {
			var elasticProvider = new ElasticFactory(new RoutingFactoryBase()).BuildCrudOperationProvider(indexName, typeName, AuthorizationStorageExtensions.AnonimousUser);
        ...
        public static void IndexWithTimestamp(object item, string indexName, string typeName, DateTime timeStamp, string userClaim)
        {
			...
			var elasticProvider = (ElasticSearchProvider)new ElasticFactory(new RoutingFactoryBase()).BuildCrudOperationProvider(indexName, typeName, AuthorizationStorageExtensions.AnonimousUser);
