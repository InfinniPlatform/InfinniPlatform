...
namespace InfinniPlatform.RestfulApi.DefaultProcessUnits
{
	public sealed class ActionUnitSetCredentials
	{
		public void Action(IApplyContext target)
		{
			...
			{
				var credentialsType = defaultBusinessProcess.Transitions[0].CredentialsType;
				if (credentialsType != null && credentialsType == AuthorizationStorageExtensions.AnonimousUserCredentials)
				{
				...
				else if (credentialsType != null && credentialsType == AuthorizationStorageExtensions.CustomCredentials)
				{
					...
					target.Context.GetComponent<IScriptRunnerComponent>()
					      .GetScriptRunner(target.Item.Configuration)
					      .InvokeScript(defaultBusinessProcess.Transitions[0].CredentialsPoint.ScenarioId, scriptArguments);
				}
			}
