...
namespace InfinniPlatform.Index.QueryLanguage.Implementation
{
	public sealed class ProjectionBuilder
	{
		...
		public JObject GetProjection(JObject jsonObject, IEnumerable<ProjectionObject> projectionItems, IEnumerable<WhereObject> where, IEnumerable<ReferencedObject> joins)
		{
			...
			foreach (var projectionItem in projectionItems)
			{
				...
				var eventDefinitions = result.SelectMany(c => c.GetResultObjectDefinitions(c.Equals(result.Last()))).ToList();
		...
		private IEnumerable<WhereObject> GetNotReferenceFields(IEnumerable<string> aliases, IEnumerable<WhereObject> @where)
		{
			var notReferencedFields = QuerySyntaxTree.GetNotAliasedFields(aliases, where.Select(w => w.RawProperty)).ToList();
