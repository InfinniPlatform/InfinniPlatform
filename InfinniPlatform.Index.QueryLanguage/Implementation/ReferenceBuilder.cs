...
namespace InfinniPlatform.Index.QueryLanguage.Implementation
{
    public sealed class ReferenceBuilder
    {
        ...
        public void FillReference(JObject jsonObject, ReferencedObject[] referencedObjects,
            IEnumerable<WhereObject> whereObjects, IFilterBuilder filterBuilder)
        {
            ...
            {
                executors.Add(reference.Path,
                    _indexFactory.BuildIndexQueryExecutor(reference.Index.ToLowerInvariant(), reference.Type, _routing));
            }
            ...
            foreach (var reference in referencedObjects)
            {
                ...
                var items =
                    executors[reference.Path].Query(FillReferencedObjectsFilter(reference, whereObjects, filterBuilder))
                        .Items;
                ...
                foreach (var identifier in identifierValue)
                {
                    ...
                    var item =
                        items.FirstOrDefault(
                            i =>
                                i.Id != null &&
                                i.Id.ToString().ToLowerInvariant() == GetIdentifierValue(identifier).ToLowerInvariant());
        ...
        private SearchModel FillReferencedObjectsFilter(ReferencedObject reference,
            IEnumerable<WhereObject> whereObjects, IFilterBuilder filterBuilder)
        {
