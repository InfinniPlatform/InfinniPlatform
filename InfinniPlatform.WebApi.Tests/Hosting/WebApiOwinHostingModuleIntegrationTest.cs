...
namespace InfinniPlatform.WebApi.Tests.Hosting
{
	...
	public sealed class WebApiOwinHostingModuleIntegrationTest
	{
		...
		public void ShouldHostWebApi()
		{
			...
			var requestUri = new Uri(string.Format("{0}://{1}:{2}/api/Test/123", HostingConfig.ServerScheme, HostingConfig.ServerName, HostingConfig.ServerPort));
			...
			var module = new WebApiOwinHostingModule(new List<Assembly>(), new ServiceRegistrationContainerFactory(templateConfig), templateConfig);
