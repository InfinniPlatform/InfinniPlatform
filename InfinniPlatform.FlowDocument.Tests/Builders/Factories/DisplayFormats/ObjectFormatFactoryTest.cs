...
namespace InfinniPlatform.FlowDocument.Tests.Builders.Factories.DisplayFormats
{
	...
	public sealed class ObjectFormatFactoryTest
	{
		...
		public void ShouldFormatObject()
		{
			...
			dynamic elementMetadata = new DynamicWrapper();
			elementMetadata.Format = "Patient: {FirstName} {LastName}. Birth date: {BirthDate:d}, weight: {Weight:N0} kg.";
			...
			// Then
			Assert.AreEqual(string.Format("Patient: {0} {1}. Birth date: {2:d}, weight: {3:N0} kg.", value.FirstName, value.LastName, value.BirthDate, value.Weight), result);
		}
		...
		public void ShouldFormatObjectWithStringProperties()
		{
			...
			// Then
			Assert.AreEqual(string.Format("Patient: {0} {1}. Birth date: {2:d}, weight: {3:N0} kg.", value.FirstName, value.LastName, DateTime.Parse(value.BirthDate), double.Parse(value.Weight)), result);
		}
	}
}