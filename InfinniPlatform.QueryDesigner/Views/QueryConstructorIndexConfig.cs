...
namespace InfinniPlatform.QueryDesigner.Views
{
    ...
    public partial class QueryConstructorIndexConfig : UserControl, IRequestExecutor, IInitializedOnLoad
    {
        ...
        public string Configuration
        {
            get
            {
                return ComboBoxConfiguration.EditValue != null
                    ? ((dynamic) ComboBoxConfiguration.EditValue).Name
                    : string.Empty;
            }
        }
        ...
        public string Document
        {
            get
            {
                return ComboBoxDocument.EditValue != null ? ((dynamic) ComboBoxDocument.EditValue).Name : string.Empty;
            }
        }
        ...
        public void OnLoad()
        {
            ...
            ComboBoxConfiguration.Properties.Items.AddRange(
                configurations.Select(c => new ImageComboBoxItem(string.Format("{0} ({1})", c.Caption, c.Name), c))
                    .ToList());
        }
        ...
        private void ComboBoxConfigurationEditValueChanged(object sender, EventArgs e)
        {
            ...
            if (ComboBoxConfiguration.EditValue != null)
            {
                IEnumerable<dynamic> documents =
                    DataProvider.GetDocuments(((dynamic) ComboBoxConfiguration.EditValue).Name);
                ComboBoxDocument.Properties.Items.AddRange(
                    documents.Select(d => new ImageComboBoxItem(string.Format("{0} ({1})", d.Name, d.Caption), d))
                        .ToList());
            }
