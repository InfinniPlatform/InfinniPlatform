...
namespace InfinniPlatform.QueryDesigner.Views
{
	...
	public partial class QueryConstructorPathToProperty : UserControl
	{
		...
		public void RefreshSchema()
		{
			...
			{
                IEnumerable<SchemaObject> paths = DataProvider.GetPropertyPaths(pathItem.Configuration,pathItem.Document,pathItem.Alias, PathResolveType);

                ComboBoxPath.Properties.Items.AddRange(paths.Select(p => new ImageComboBoxItem(
                    string.IsNullOrEmpty(pathItem.Alias) ? p.GetFullPath() : pathItem.Alias + "." + p.GetFullPath(), p)).ToList());
			}
