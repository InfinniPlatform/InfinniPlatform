...
{
	public sealed class DataProviderStandard : IDataProvider
	{
		public IEnumerable<dynamic> GetConfigurationList()
		{
			return ManagerFactoryConfiguration.BuildConfigurationMetadataReader().GetItems();
		}

		public IEnumerable<dynamic> GetDocuments(string configurationId)
		{
			return new ManagerFactoryConfiguration(configurationId).BuildDocumentMetadataReader().GetItems();
		}

		public dynamic GetDocumentSchema(string configuration, string document)
		{
			if (!string.IsNullOrEmpty(configuration) && !string.IsNullOrEmpty(document))
			{
				return new ManagerFactoryConfiguration(configuration).BuildDocumentMetadataReader().GetItem(document).Schema;
			}
			return null;
		}
