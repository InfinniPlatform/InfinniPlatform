...
namespace InfinniPlatform.Index.ElasticSearch.Tests.ElasticWrappers
{
	...
	public class ElasticSearchBehavior
	{
		...
		{
			var elasticSettings = new ConnectionSettings(new Uri("http://localhost:9200")).SetDefaultIndex("testindex");
			var client = new ElasticClient(elasticSettings);
        ...
        public void ShouldReindexWithoutDataLostTest()
        {
			...
			var queryExecutor = new ElasticFactory(new RoutingFactoryBase()).BuildIndexQueryExecutor("testperson", "testperson", AuthorizationStorageExtensions.AnonimousUser);
		...
		public void ShouldFilterOnFields()
		{
			...
			try
			{
                ...
                if (client.Connection.HeadSync(new Uri("http://127.0.0.1:9200")).Success)
				{
					...
					var item = client.Get<TestPerson>("345243");
