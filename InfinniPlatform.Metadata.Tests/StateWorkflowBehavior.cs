...
namespace InfinniPlatform.Metadata.Tests
{
	...
	public sealed class StateWorkflowBehavior
	{
		...
		public void ShouldInvokeTransitionsForWorkflow(bool runWithFlow)
		{
			...
			Action<dynamic> actionFlow = (context) =>
				                    {
					                    target.ActionFlowExecuted = "Executed";
				                    };

			Action<dynamic> actionWithoutFlow = context =>
				                                    {
					                                    target.ActionWithoutFlowExecuted = "Executed";
				                                    };

			var workflowConfig = new StateWorkflowStartingPointConfig();
			workflowConfig
				.ForState("Temporary", st => st.Move(transition => transition.WithAction(() => new ActionOperator("TemporaryAction", actionFlow))))
				.FlowWithoutState(st => st.Move(tr => tr.WithAction(() => new ActionOperator("ActionWithoutFlow", actionWithoutFlow))));
