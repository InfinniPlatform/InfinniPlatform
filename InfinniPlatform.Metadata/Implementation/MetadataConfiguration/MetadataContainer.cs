...
namespace InfinniPlatform.Metadata.Implementation.MetadataConfiguration
{
	...
	{
        private SearchAbilityType _searchAbility;
        ...
        /// </summary>
        public SearchAbilityType SearchAbility
        {
            get { return _searchAbility; }
        }
        ...
        {
            var existingProcess = _processes.FirstOrDefault(v => v.Name.ToLowerInvariant() == process.Name.ToLowerInvariant());
        ...
        {
            var existingScenario = _scenario.FirstOrDefault(v => v.Name.ToLowerInvariant() == scenario.Name.ToLowerInvariant());
        ...
        {
            var existingService = _services.FirstOrDefault(v => v.Name.ToLowerInvariant() == service.Name.ToLowerInvariant());
		...
		{
            var existingGenerator = _generators.FirstOrDefault(v => v.Name.ToLowerInvariant() == generator.Name.ToLowerInvariant());
        ...
        {
            var existingWarning = _validationWarnings.FirstOrDefault(v => v.Name.ToLowerInvariant() == warning.Name.ToLowerInvariant());
        ...
        {
            var existingError = _validationErrors.FirstOrDefault(v => v.Name.ToLowerInvariant() == error.Name.ToLowerInvariant());
        ...
        {
            var existingStatus = _statuses.FirstOrDefault(v => v.Name.ToLowerInvariant() == status.Name.ToLowerInvariant());
		...
		{
			var existingPrintView = _printViews.FirstOrDefault(v => string.Equals(v.Name, printView.Name, StringComparison.OrdinalIgnoreCase));
        ...
        {
            var existingProcess = _processes.FirstOrDefault(v => v.Name.ToLowerInvariant() == processName.ToLowerInvariant());
            if (existingProcess != null)
            {
                _processes.Remove(existingProcess);
            }
        ...
        {
            var existingScenario = _scenario.FirstOrDefault(v => v.Name.ToLowerInvariant() == scenarioName.ToLowerInvariant());
            if (existingScenario != null)
            {
                _scenario.Remove(existingScenario);
            }
        ...
        {
            var existingService = _services.FirstOrDefault(v => v.Name.ToLowerInvariant() == serviceName.ToLowerInvariant());
            if (existingService != null)
            {
                _services.Remove(existingService);
            }
		...
		{
            var existingGenerator = _generators.FirstOrDefault(v => v.Name.ToLowerInvariant() == generatorName.ToLowerInvariant());
            if (existingGenerator != null)
            {
                _generators.Remove(existingGenerator);
            }
        ...
        {
            var existingWarning = _validationWarnings.FirstOrDefault(v => v.Name.ToLowerInvariant() == warningName.ToLowerInvariant());
            if (existingWarning != null)
            {
                _validationWarnings.Remove(existingWarning);
            }
        ...
        {
            var existingError = _validationErrors.FirstOrDefault(v => v.Name.ToLowerInvariant() == errorName.ToLowerInvariant());
            if (existingError != null)
            {
                _validationErrors.Remove(existingError);
            }
        ...
        {
            var existingStatus = _statuses.FirstOrDefault(v => v.Name.ToLowerInvariant() == statusName.ToLowerInvariant());
            if (existingStatus != null)
            {
                _statuses.Remove(existingStatus);
            }
		...
		{
			var existingPrintView = _printViews.FirstOrDefault(v => string.Equals(v.Name, printViewName, StringComparison.OrdinalIgnoreCase));
