...
namespace InfinniPlatform.Metadata.Implementation.MetadataConfiguration
{
	...
	public class MetadataConfiguration : IMetadataConfiguration
	{
	    ...
	    {
            var existingRegister = _registers.FirstOrDefault(v => v.Name.ToLowerInvariant() == registerName.ToLowerInvariant());
		...
		public string GetExtensionPointValue(string metadataId,  string actionInstanceName, string extensionPointTypeName)
		{
			...
			var actionHandlerInstance = ServiceRegistrationContainer.GetRegistrationByInstanceName(metadataId, actionInstanceName);
			...
			throw new ArgumentException(string.Format("Metadata extension point for point: {0} and container: {1} not registered",extensionPointTypeName,metadataId));
		}
