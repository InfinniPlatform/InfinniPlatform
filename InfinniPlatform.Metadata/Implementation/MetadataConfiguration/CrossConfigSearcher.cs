...
namespace InfinniPlatform.Metadata.Implementation.MetadataConfiguration
{
    public sealed class CrossConfigSearcher : ICrossConfigSearcher
    {
        ...
        public dynamic GetDocuments(IEnumerable<object> filterObject, int pageNumber, int pageSize, string routing, IEnumerable<object> sorting, IEnumerable<string> configs = null, IEnumerable<string> documents = null)
        {
            var elasticProvider = _indexFactory.BuildMultiIndexDocumentProvider(routing, indexNames: configs, typeNames: documents);
            return elasticProvider.GetDocument(filterObject, pageNumber, pageSize, sortingDescription: sorting);
        }
    }
}