...
namespace InfinniPlatform.PrintViewDesigner.Controls.PrintViewTreeBuilders
{
    internal static class BuildHelper
    {
        ...
        {
            "Span", "Bold", "Italic", "Underline", "Hyperlink", "LineBreak",
            "Run", "Image", "BarcodeEan13", "BarcodeQr"
        };
            ...
            return (childElementNode, accept) =>
            {
                if (parentElementNode.CanDeleteChild.TryInvoke(childElementNode, accept))
                {
                    if (!accept || Helpers.AcceptQuestionMessage(Resources.DeleteElementQuestion, childElementNode))
                    {
                        ...
                        ProcessChildElement(childElements, childElementNode, childElements.RemoveItem,
                            wrapElementMetadata);
            ...
            return () =>
            {
                if (parentElementNode.CanPaste.TryInvoke())
                {
                    ...
                    if (entry != null)
                    {
                        ...
                        var newChildElementNode = new PrintElementNode(parentElementNode, childElementNode.ElementType,
                            childElementNode.ElementMetadata.Clone());
        ...
        private static IList GetChildElementCollection(PrintElementNode parentElementNode, string childElementCollection,
            bool createIfNull)
        {
