...
namespace InfinniPlatform.PrintViewDesigner.Controls
{
	static class Helpers
	{
		...
		{
			return string.Join(".", properties.Select(p => (p != null) ? p.ToString() : null).Where(p => !string.IsNullOrEmpty(p)));
		}
		...
		{
			MessageBox.Show(string.Format(message, args), Resources.PrintViewDesignerName, MessageBoxButton.OK, MessageBoxImage.Warning);
		}

		public static bool AcceptQuestionMessage(string message, params object[] args)
		{
			return MessageBox.Show(string.Format(message, args), Resources.PrintViewDesignerName, MessageBoxButton.YesNo, MessageBoxImage.Warning, MessageBoxResult.No) == MessageBoxResult.Yes;
		}
	}
}