...
namespace InfinniPlatform.Json
{
	...
	public sealed class JsonCollectionSearchByPropertyProvider : IJsonTokenProvider
	{
		...
		public JsonCollectionSearchByPropertyProvider(string token)
		{
			...
			{
				throw new ArgumentException(string.Format("fail to get property search value. No value specified: {0}", token));
			}
		...
		public IEnumerable<ParsedToken> GetJsonToken(ParsedToken parsedToken)
		{
			...
			else
			{
				...
				itemByPropertyValue = parsedToken.JToken.Children().FirstOrDefault(j => ContainsPropertyName(j, _propertyName) && GetValue(j) == _propertyValue);
				if (itemByPropertyValue == null)
				{
					itemByPropertyValue = parsedToken.JToken.Children().OfType<JObject>()
					   .Select(j => j.Children().FirstOrDefault(ch => ContainsPropertyName(ch, _propertyName) && GetValue(ch) == _propertyValue))
					   .FirstOrDefault();
