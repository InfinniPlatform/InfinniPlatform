...
namespace InfinniPlatform.Json
{
	public class JsonParser
	{
		...
		public JsonParser()
		{
			...
			_jsonTokenConfiguration.BuilderFor(token => int.TryParse(token, out collectionItemToken), token => new JsonCollectionItemTokenProvider(token));
        ...
        public IEnumerable<ParsedTokenContainer> FindJsonPath(JToken startToken, string path)
		{
		    ...
		    {
		        GetResultJsonTokenList(tokenList.Select(tl => new ParsedToken(tl.TokenName,tl.JToken)).ToList(), tokenList, jsonTokenProvider);               
	    ...
	    private static void GetResultJsonTokenList(IEnumerable<ParsedToken> startToken, List<ParsedToken> jTokenResult, IJsonTokenProvider jsonTokenProvider)
	    {
            jTokenResult.Clear();
