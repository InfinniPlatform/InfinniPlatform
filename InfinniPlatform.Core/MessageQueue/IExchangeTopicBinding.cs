...
namespace InfinniPlatform.MessageQueue
{
	...
	public interface IExchangeTopicBinding
	{
		...
		/// <remarks>
		/// При формировании шаблона маршрутизации используются следующие соглашения. Шаблон должен быть пустым, либо состоять из слов,
		/// разделенных точками. Каждое слово может содержать символы из набора [a-zA-Z0-9]. Дополнительно могут быть использованы
		/// специальные символы: * - означает одно слово, # - означает любое количество слов.
		/// </remarks>
		void Subscribe(string queue, Func<IQueueConsumer> consumer, string routingPattern, Action<IQueueConfig> config = null);
