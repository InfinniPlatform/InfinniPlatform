	...
	public sealed class ReflectedParameterInfo
	{
		...
		private object ValueToType(object value)
		{
			...
			if (_parameterType == typeof(string))
				...
				value = string.IsNullOrEmpty(value.ToString()) ? new DateTime?() : Convert.ToDateTime(value, CultureInfo.CurrentCulture);
		...
		private static bool TypeIsDynamicCollection(Type target)
		{
			return target.IsGenericType
				   ...
				   && (DynamicType.IsAssignableFrom(target.GetGenericArguments().First()) || target.GetGenericArguments().First() == ObjectType);
		}
