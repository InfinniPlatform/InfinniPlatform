...
namespace InfinniPlatform.Api.Metadata.ConfigurationManagers.File
{
    ...
    public sealed class JsonFileConfigManager : IManagerIdentifiers
    {
        ...
        public void ReadConfigurations()
        {
            ...
            var fileNames = Directory.GetFiles(_configDirectory).Where(f => Path.GetExtension(f).ToLowerInvariant() == ".zip").ToList();
            ...
            foreach (var fileName in fileNames)
            {
                ...
                {
                    Console.WriteLine(string.Format("Error on reading json configuration file archive: {0}, Error: {1}", fileName, e.Message));
                }
        ...
        public IDataReader BuildDocumentElementReader(string configurationId, string documentName, string metadataType)
        {
            ...
            if (jsonConfig != null)
            {
                ...
                dynamic document = documents.FirstOrDefault(d => d.Name.ToLowerInvariant() == documentName.ToLowerInvariant());
                if (document != null)
                {
