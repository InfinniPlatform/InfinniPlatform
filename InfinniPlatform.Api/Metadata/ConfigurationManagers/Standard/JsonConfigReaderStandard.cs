...
namespace InfinniPlatform.Api.Metadata.ConfigurationManagers.Standard
{
    public sealed class JsonConfigReaderStandard : IJsonConfigReader
    {
		...
		{
			return new ManagerFactoryDocument(configurationId, documentName).BuildValidationWarningsMetadataReader();
		}

		public IDataReader GetValidationErrorsReader(string configurationId, string documentName)
		{
			return new ManagerFactoryDocument(configurationId, documentName).BuildValidationErrorsMetadataReader();
		}
        ...
        {
            var manager = new ManagerFactoryDocument(configurationId, documentName).BuildManagerByType(metadataType);
