...
namespace InfinniPlatform.Api.Metadata.ConfigurationManagers.Standard.MetadataManagers
{
	public sealed class MetadataManagerDocument  : IDataManager
	{
		...
		public MetadataManagerDocument(string version, string configurationId, IMetadataContainerInfo metadataContainerInfo, string metadataType)
		{
		    _version = version;
		    _configurationId = configurationId;
		    var metadataCacheRefresher = new MetadataCacheRefresher(version, configurationId, metadataContainerInfo.GetMetadataContainerName(), metadataType);
            _manager = new MetadataManagerElement(version, new ManagerIdentifiersStandard().GetConfigurationUid(_version, _configurationId), metadataCacheRefresher, 
                new MetadataReaderConfigurationElement(_version, _configurationId, new MetadataContainerDocument()), metadataContainerInfo, metadataType);
		}
		...
		public void DeleteItem(dynamic metadataObject)
		{
		    var document =
		        MetadataExtensions.GetStoredMetadata(
		            new ManagerFactoryConfiguration(_version, _configurationId).BuildDocumentMetadataReader(), metadataObject);
