...
namespace InfinniPlatform.Api.Schema
{
	public sealed class SchemaCreator
	{
		...
		private DataSchema FindProperty(DataSchema partialObject, SchemaObject findObject)
		{
			...
			var path = findObject.ParentPath.Split(new char[] { '.' }, StringSplitOptions.RemoveEmptyEntries).ToList();
			path.AddRange(findObject.Name.Split(new char[] { '.' }, StringSplitOptions.RemoveEmptyEntries));

			var schemaToFind = partialObject;

			bool isCollectionItemPath = false;
