...
namespace InfinniPlatform.Api.Index.SearchOptions
{
	...
	public sealed class QueryCriteriaInterpreter
	{
		...
		public QueryCriteriaInterpreter()
		{
			...
			_criteriaInterpreter.Add(CriteriaType.IsNotEquals, value => new NotValidator { Operator = new EqualValidator { Value = value } });

			_criteriaInterpreter.Add(CriteriaType.IsEmpty, value => new NullOrEmptyValidator());
			_criteriaInterpreter.Add(CriteriaType.IsNotEmpty, value => new NotValidator { Operator = new NullOrEmptyValidator() });

			_criteriaInterpreter.Add(CriteriaType.IsContains, value => new ContainsValidator { Value = ToStringOrEmpty(value) });
			_criteriaInterpreter.Add(CriteriaType.IsNotContains, value => new NotValidator { Operator = new ContainsValidator { Value = ToStringOrEmpty(value) } });

			_criteriaInterpreter.Add(CriteriaType.IsStartsWith, value => new StartsWithValidator { Value = ToStringOrEmpty(value) });
			_criteriaInterpreter.Add(CriteriaType.IsNotStartsWith, value => new NotValidator { Operator = new StartsWithValidator { Value = ToStringOrEmpty(value) } });

			_criteriaInterpreter.Add(CriteriaType.IsEndsWith, value => new EndsWithValidator { Value = ToStringOrEmpty(value) });
			_criteriaInterpreter.Add(CriteriaType.IsNotEndsWith, value => new NotValidator { Operator = new EndsWithValidator { Value = ToStringOrEmpty(value) } });

			_criteriaInterpreter.Add(CriteriaType.IsMoreThan, value => new MoreThanValidator { Value = value });
			_criteriaInterpreter.Add(CriteriaType.IsMoreThanOrEquals, value => new MoreThanOrEqualValidator { Value = value });
		...
		public IEnumerable<dynamic> ApplyFilter(IEnumerable<dynamic> filter, IEnumerable<dynamic> result)
		{
			...
			return result.Where(r => criteriaList.All(c => c.Operator.Validate(ObjectHelper.GetProperty(r,c.Criteria.Property)))).ToList();
		}
	}
}