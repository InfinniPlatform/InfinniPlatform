...
namespace InfinniPlatform.Api.Packages.ConfigStructure
{
    public sealed class DirectoryStructure : IConfigStructure
    {
        ...
        public void AddConfiguration(IEnumerable<string> configuration)
        {
            ...
            {
                Directory.CreateDirectory(Path.Combine(_folderToStructure, ConfigurationFixtureNames.GetConfigurationFolder()));
            }

            File.WriteAllLines(Path.Combine(_folderToStructure,ConfigurationFixtureNames.GetConfigurationFileName()),configuration);
        }

        public void AddDocument(string documentName, IEnumerable<string> document)
        {
            if (!Directory.Exists(Path.Combine(_folderToStructure,ConfigurationFixtureNames.GetDocumentFolderName(documentName))))
            {
                Directory.CreateDirectory(Path.Combine(_folderToStructure,ConfigurationFixtureNames.GetDocumentFolderName(documentName)));
            }
        ...
        {
            if (!Directory.Exists(Path.Combine(_folderToStructure, ConfigurationFixtureNames.GetRegisterFolderName(registerName))))
            {
                Directory.CreateDirectory(Path.Combine(_folderToStructure, ConfigurationFixtureNames.GetRegisterFolderName(registerName)));
            }
        ...
        {
			if (!Directory.Exists(Path.Combine(_folderToStructure, ConfigurationFixtureNames.GetMenuFolderName(menuName))))
			{
				Directory.CreateDirectory(Path.Combine(_folderToStructure, ConfigurationFixtureNames.GetMenuFolderName(menuName)));
			}


			var currentMenuFolder = ConfigurationFixtureNames.GetMenuFolderName(menuName);

			string menuFileName = ConfigurationFixtureNames.GetDocumentFileName(currentMenuFolder, menuName);
        ...
        {
            if (!Directory.Exists(Path.Combine(_folderToStructure,ConfigurationFixtureNames.GetAssemblyFolderName(assemblyName))))
            {
                Directory.CreateDirectory(Path.Combine(_folderToStructure,ConfigurationFixtureNames.GetDocumentFolderName(assemblyName)));
            }
	    ...
	    {
			if (!Directory.Exists(Path.Combine(_folderToStructure, ConfigurationFixtureNames.GetReportFolderName(reportName))))
			{
				Directory.CreateDirectory(Path.Combine(_folderToStructure, ConfigurationFixtureNames.GetReportFolderName(reportName)));
			}
        ...
        {
            if (!Directory.Exists(Path.Combine(_folderToStructure,ConfigurationFixtureNames.GetMetadataTypeFolder(document,metadataType,metadataName))))
            {
                Directory.CreateDirectory(Path.Combine(_folderToStructure,ConfigurationFixtureNames.GetMetadataTypeFolder(document, metadataType,
                                                                                          metadataName)));
            }
        ...
        {
			return File.ReadAllText(Path.Combine(_folderToStructure,ConfigurationFixtureNames.GetConfigurationFileName())).ToDynamic();
        }
        ...
        public dynamic GetDocument(string documentName)
        {
			...
			return File.ReadAllText(Path.Combine(_folderToStructure,ConfigurationFixtureNames.GetDocumentFileName(documentFolder, documentName))).ToDynamic();
        }
        ...
        public dynamic GetRegister(string registerName)
        {
            ...
            return File.ReadAllText(Path.Combine(_folderToStructure, ConfigurationFixtureNames.GetRegisterFileName(registerFolder, registerName))).ToDynamic();
        }
        ...
        public dynamic GetDocumentMetadataType(string document, string metadataName, string metadataType)
        {
			...
			return File.ReadAllText(Path.Combine(_folderToStructure,ConfigurationFixtureNames.GetMetadataTypeFileName(currentMetadataTypeFolder, metadataName))).ToDynamic();
        }
	    ...
	    public dynamic GetMenu(string menuName)
	    {
			...
			return File.ReadAllText(Path.Combine(_folderToStructure, ConfigurationFixtureNames.GetMenuFileName(menuFolder, menuName))).ToDynamic();
	    }
	    ...
	    public dynamic GetReport(string reportName)
	    {
			...
			return File.ReadAllText(Path.Combine(_folderToStructure, ConfigurationFixtureNames.GetReportFileName(reportFolder,reportName))).ToDynamic();
	    }
    }
}
