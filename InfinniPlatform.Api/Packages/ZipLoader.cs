...
namespace InfinniPlatform.Api.Packages
{
	public static class ZipLoader
	{
		...
		public static ZipArchive ReadArchive(string filePath, FileStream fileStream, Encoding zipEncoding)
		{
			...
			{
				throw new ArgumentException(string.Format("Error on reading json configuration file archive: {0}, Error: {1}",
				                                          filePath, e.Message));
			}
		...
		public static void CreateZipArchiveFromDirectory(string directoryPath, string archiveName)
		{
			if (File.Exists(Path.Combine(Directory.GetCurrentDirectory(), archiveName)))
			{
				File.Delete(Path.Combine(Directory.GetCurrentDirectory(), archiveName));
			}
			ZipFile.CreateFromDirectory(directoryPath, Path.Combine(Directory.GetCurrentDirectory(), archiveName));
		}
		...
		{
			var folderNames = folderName.Split(new char[] { '/' },StringSplitOptions.RemoveEmptyEntries);
			var folderNamesParent = folderParent.Split(new char[] { '/' }, StringSplitOptions.RemoveEmptyEntries);
			if (folderNames.Count() < folderNamesParent.Count())
			{
				return false;
			}

			for (int i = 0; i < folderNamesParent.Count(); i++)
			{
