...
namespace InfinniPlatform.DesignControls.PropertyDesigner
{
	public partial class CollectionEditor : UserControl
	{
			...
			foreach (var o in items)
			{
				...
				foreach (var property in _rows[instance].Properties)
				{
					...
					if (objectProperty != null)
					{
						...
						DesignerExtensions.SetSimplePropertiesFromInstance(objectProperty.SimpleProperties, propertyValue);
						DesignerExtensions.SetCollectionPropertiesFromInstance(objectProperty.CollectionProperties, propertyValue);
					...
					{
					    var arr = DynamicWrapperExtensions.ToDynamicList(ObjectHelper.GetProperty(o, property.Key).ToString());
					...
					else
					{
						column.ColumnEdit = _repository.GetRepositoryItem(property.Key, property.Value) ??
						                    (!string.IsNullOrEmpty(PropertyName)
							                     ? _repository.GetRepositoryItem(PropertyName + "." + property.Key, property.Value)
							                     : null);
		...
		private Dictionary<dynamic, CollectionProperty> _rows = new Dictionary<dynamic, CollectionProperty>();
		...
		private void PropertiesView_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
		{
		...
		private void repositoryItemButtonEdit_ButtonClick(object sender, ButtonPressedEventArgs e)
		{
			...
			else if (e.Button.Kind == ButtonPredefines.Ellipsis)
			{
				...
				ObjectProperty valueProperty = property.Properties[PropertiesView.FocusedColumn.FieldName] as ObjectProperty;


				dynamic propertyValue = null;
				try
				{
					propertyValue = DynamicWrapperExtensions.ToDynamic((string)ObjectHelper.GetProperty(instance, PropertiesView.FocusedColumn.FieldName).ToString());
					DesignerExtensions.SetSimplePropertiesFromInstance(valueProperty.SimpleProperties, propertyValue);
					DesignerExtensions.SetCollectionPropertiesFromInstance(valueProperty.CollectionProperties, propertyValue);

				}
				catch (Exception)
				{
					propertyValue = DynamicWrapperExtensions.ToDynamicList((string)ObjectHelper.GetProperty(instance, PropertiesView.FocusedColumn.FieldName).ToString());
				}
