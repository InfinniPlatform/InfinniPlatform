...
namespace InfinniPlatform.DesignControls.Controls.Properties
{
	public static class ActionElement
	{
		...
		private static ObjectProperty GetPopupButtonProperties(int actionLevel)
		{
			...
			if (actionLevel > 3)
			{
                return new ObjectProperty(new Dictionary<string, IControlProperty>(), new Dictionary<string, CollectionProperty>());
			}
            return new ObjectProperty(new Dictionary<string, IControlProperty>()
		        {
		            ...
		            {
		                {"Items", new CollectionProperty(new Dictionary<string, IControlProperty>().GetActionElements(actionLevel))}
		            });
		}


        public static Dictionary<string, IControlProperty> GetActionProperties(this Dictionary<string, IControlProperty> properties)
		{
			properties.Add("Action", new ObjectProperty(Action.GetActions(), new Dictionary<string, CollectionProperty>()));
			return properties;
		}

        public static Dictionary<string, IControlProperty> GetActionElements(this Dictionary<string, IControlProperty> properties, int currentActionElementLevel = 0)
		{
