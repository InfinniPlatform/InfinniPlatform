...
{
	/// <summary>
	///   Форма дизайнера выступает в качестве медиатора между инспектором объектов, панелью размещения контролов и репозиторием контролов
	/// </summary>
	public partial class DesignerControl : UserControl
	{
		...
		private void InitObjectInspectorTree()
		{
			...
			_objectInspectorPopupMenu.Form = this.FindForm();
		...
		{
			if (MessageBox.Show("Clear all layout?", "Need confirm", MessageBoxButtons.OKCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.OK)
			{
				ClearControls();
			}
		}
