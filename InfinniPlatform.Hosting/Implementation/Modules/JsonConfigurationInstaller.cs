...
namespace InfinniPlatform.Hosting.Implementation.Modules
{
	...
	public sealed class JsonConfigurationInstaller
	{
		...
		public void InstallConfiguration(IMetadataConfiguration metadataConfiguration)
		{
			...
			foreach (var document in _documents.Where(d => d.ConfigId == metadataConfiguration.ConfigurationId).ToList())
			{
				try
				{
					...
					RegisterValidators(
						_validationWarnings.Where(pr => pr.DocumentId == document.Name && pr.ConfigId == metadataConfiguration.ConfigurationId).ToList(),
		...
		private void RegisterGenerators(IEnumerable<dynamic> generators, IMetadataConfiguration metadataConfiguration, string metadataName)
		{
			foreach (var generator in generators)
			{
				try
				{
					...
					Logger.Log.Info("Config:{0}, Document: {1}, Generator: {2} registered", metadataConfiguration.ConfigurationId, metadataName, generator.Name);
				}
				catch (Exception e)
				{
		...
		private void RegisterValidators(IEnumerable<dynamic> validators, IMetadataConfiguration metadataConfiguration, string metadataName, bool isError)
		{
			foreach (var validator in validators)
			{
				try
				{
					...
					Logger.Log.Info("Config:{0}, Document: {1}, Validator: {2} registered", metadataConfiguration.ConfigurationId, metadataName, validator.Name);
				}
				catch (Exception e)
				{
		...
		private void RegisterViews(IEnumerable<dynamic> views, IMetadataConfiguration metadataConfiguration, string metadataName)
		{
			foreach (var view in views)
			{
				try
				{
					...
					Logger.Log.Info("Config: {0}, Document: {1}, View: {2} registered", metadataConfiguration.ConfigurationId, metadataName, view.Name);
				}
				catch (Exception e)
				{
		...
		private void RegisterPrintViews(IEnumerable<dynamic> printViews, IMetadataConfiguration metadataConfiguration, string metadataName)
		{
			foreach (var printView in printViews)
			{
				try
				{
					...
					Logger.Log.Info("Config: {0}, Document: {1}, PrintView: {2} registered", metadataConfiguration.ConfigurationId, metadataName, printView.Name);
				}
				catch (Exception e)
				{
		...
		private void RegisterSchema(IMetadataConfiguration metadataConfiguration, dynamic document)
		{
			try
			{
				...
				Logger.Log.Info("Config:{0}, Document: {1}, document schema registered", metadataConfiguration.ConfigurationId, document.Name);
			}
			catch (Exception e)
			{
		...
		private void RegisterProcesses(IEnumerable<dynamic> processes, IMetadataConfiguration metadataConfiguration, string metadataName)
		{		
	        ...
	        string metadataName)
	    {
	        foreach (var service in services)
	        {
	            try
	            {
	                ...
	                {
	                    foreach (var extensionPoint in DynamicWrapperExtensions.ToEnumerable(localService.ExtensionPoints))
	                    {
	                ...
	                Action<IServiceRegistration> registrationAction =
	                    reg => ExtensionPointHandlerExtensions.RegisterHandlerInstance(reg, localService.Name, instanceAction)
                            .SetResultHandler(HttpResultHandlerType.BadRequest);

	                metadataConfiguration.ServiceRegistrationContainer.AddRegistration(metadataName, service.Type.Name, registrationAction);

					Logger.Log.Info("Config:{0}, Document{1}, Service: {2} registered", metadataConfiguration.ConfigurationId,metadataName, service.Name);
	            }
	            catch (Exception e)
	            {
		...
		private static void SetCommonOptions(IMetadataConfiguration metadataConfiguration, dynamic fullDocument)
		{
			try
			{
				...
				{
					metadataConfiguration.SetSearchAbilityType(fullDocument.Name, (SearchAbilityType)fullDocument.SearchAbility);
				}
				else
				{
			...
			{
				Logger.Log.Error("Fail to register common metadata options: {0}. Error: {1}", fullDocument.Name, e.Message);
			}
		}
	    ...
	    private static void RegisterScenario(IEnumerable<dynamic> scenarios, IMetadataConfiguration metadataConfiguration, string metadataName)
		{
