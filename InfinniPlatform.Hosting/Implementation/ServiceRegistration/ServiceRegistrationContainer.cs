...
namespace InfinniPlatform.Hosting.Implementation.ServiceRegistration
{
    ...
    public sealed class ServiceRegistrationContainer : IServiceRegistrationContainer
    {
        ...
        public ServiceRegistrationContainer(IServiceTemplateConfiguration serviceTemplateConfiguration,
        ...
        /// <returns></returns>
        public IServiceRegistrationContainer AddRegistration(string metadata, string serviceName,
            Action<IServiceRegistration> registrationAction = null,
            Func<string, IExtensionPointHandler> actionHandler = null)
        {
            //необходимо добавить проверку уникальности сервисов

            var registration = new ServiceRegistration(metadata, serviceName,
                _serviceTemplateConfiguration.GetServiceHandler(serviceName));
        ...
        {
            return
                _serviceRegistrations.FirstOrDefault(
                    sr =>
                        sr.MetadataName.ToLowerInvariant() == metadata.ToLowerInvariant() &&
                        sr.ServiceName.ToLowerInvariant() == service.ToLowerInvariant());
        }
        ...
        {
            var registration =
                _serviceRegistrations.Where(sr => sr.MetadataName.ToLowerInvariant() == metadata.ToLowerInvariant())
                    .ToList();
