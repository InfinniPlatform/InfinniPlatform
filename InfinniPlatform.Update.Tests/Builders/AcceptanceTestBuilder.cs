...
namespace InfinniPlatform.Update.Tests.Builders
{
	public static class AcceptanceTestBuilder
	{
		...
		{
            var jsonEnumerable = new GZipDataCompressor().ReadAsJsonEnumerable(pathToArchive);
		...
		public static string GenerateSnils()
		{
			...
			var input = random.Next(100000000, 200000000).ToString();

			var actualCheckSum = input.Reverse().Select((c, index) => (index + 1) * (int)char.GetNumericValue(c)).Sum() % 101;
			var checkSum = actualCheckSum.ToString("00", CultureInfo.InvariantCulture);
